...
Future<UserModel> makeLoginRequest_(String email, String password) async {
  ...
  Response response = await post(Api.LOGIN, headers: config, body: jsonData);
...
Future<String> makeRegisterRequest_(String email, String password) async {
  var jsonData = json.encode({"email": email, "password": password});
  Response response = await post(Api.REGISTER, headers: config, body: jsonData);
...
Future<String> checkEmailRequest_(String email) async {
  ...
  Response response =
      await post(Api.CHECKEMAIL, headers: config, body: jsonData);
  return response.body;
}
